swg_multisite_obs_SEPPR <- function(nsim, sim_st_yr, sim_end_yr) {
  ##########################################
  ##
  ## Simulations
  ##
  ...
  for(yr.to.sim in sim_st_yr:sim_end_yr) {
    ...
    for(sea_sim_num in 1:4) {
      ## simulate an arbitrary number of realizations of a season
      ## define some variables for simulation
      ...
      if(sea_sim_num == 1) {
        #for(k in 1:nsim) ST1.sim[k, ] = rep(seatot[which(uyr == yr.to.sim), sea_sim_num], length(ct.sim))
        for(k in 1:nsim) ST1.sim[k, ] = rep(seatot[which(uyr == yr.to.sim), sea_sim_num], length(ct.sim))
      } else if(sea_sim_num == 2) {
        for(k in 1:nsim) ST2.sim[k, ] = rep(seatot[which(uyr == yr.to.sim), sea_sim_num], length(ct.sim))
      } else if(sea_sim_num == 3) {
        for(k in 1:nsim) ST3.sim[k, ] = rep(seatot[which(uyr == yr.to.sim), sea_sim_num], length(ct.sim))
      } else if(sea_sim_num == 4) {
        for(k in 1:nsim) ST4.sim[k, ] = rep(seatot[which(uyr == yr.to.sim), sea_sim_num], length(ct.sim))
      }
      ...
      if(sea_sim_num == 1) {
        for(k in 1:nsim) SMX1.sim[k, ] = rep(seamax[which(uyr == yr.to.sim), sea_sim_num], length(ct.sim))
      } else if(sea_sim_num == 2) {
        for(k in 1:nsim) SMX2.sim[k, ] = rep(seamax[which(uyr == yr.to.sim), sea_sim_num], length(ct.sim))
      } else if(sea_sim_num == 3) {
        for(k in 1:nsim) SMX3.sim[k, ] = rep(seamax[which(uyr == yr.to.sim), sea_sim_num], length(ct.sim))
      } else if(sea_sim_num == 4) {
        for(k in 1:nsim) SMX4.sim[k, ] = rep(seamax[which(uyr == yr.to.sim), sea_sim_num], length(ct.sim))
      }
      ...
      if(sea_sim_num == 1) {
        for(k in 1:nsim) SMN1.sim[k, ] = rep(seamin[which(uyr == yr.to.sim), sea_sim_num], length(ct.sim))
      } else if(sea_sim_num == 2) {
        for(k in 1:nsim) SMN2.sim[k, ] = rep(seamin[which(uyr == yr.to.sim), sea_sim_num], length(ct.sim))
      } else if(sea_sim_num == 3) {
        for(k in 1:nsim) SMN3.sim[k, ] = rep(seamin[which(uyr == yr.to.sim), sea_sim_num], length(ct.sim))
      } else if(sea_sim_num == 4) {
        for(k in 1:nsim) SMN4.sim[k, ] = rep(seamin[which(uyr == yr.to.sim), sea_sim_num], length(ct.sim))
      }
      ...
      for(i in 1:nsim) {
        ...
        for(k in 1:np) {
          ...
          if(sea_cov == "with") {
            ...
            SC[ , k] <- exp(apply(coefamt[ , k]*rbind(1, ct.sim, st.sim), FUN = sum, MAR = 2, na.rm = T))/SH[k]
          }
        }
        ...
        for(d in 1:nt.sim) { 
          ...
          if(sea_cov == "with") {
            ...
            SIMmax[i, d, ] <- apply(rbind(1, SIMmin.old, SIMmax.old, ct.sim[d], st.sim[d], SIMocc[i, d, ], Rt.sim[d])*coefmax, 2, sum) + mvrnorm(1, mu = rep(0, np), Sigma = TEMPcov[[mo.sim[d]]])
          }
          ...
          if(sea_cov == "with") {
            ...
            SIMmin[i, d, ] <- apply(rbind(1, SIMmin.old, SIMmax.old, ct.sim[d], st.sim[d], SIMocc[i, d, ], Rt.sim[d])*coefmin, 2, sum) + mvrnorm(1, mu = rep(0, np), Sigma = TEMPcov[[mo.sim[d]]])
          }
          # can't have max temp less than min temp
